...
from ._nanfuncs import _fill_doc, _nanrank, _promote_to_real
...
# and filled into each docstring's ``{params}`` placeholder by ``_fill_doc``
# (borrowed from ``_nanfuncs``, which uses the same mechanism). ``{extra}``
# is where a function's own extra parameter goes, so that the rendered
# Parameters section keeps signature order.
_COMMON_PARAMS = """\
x : array
...
def _window_doc(dtype=_PROMOTED_DTYPE, extra=""):
    """
    Build the `_fill_doc` decorator for one function in this module.
